  ...
  private[features] def toDataEntries(dataFrames: List[NamedDataFrame]) : List[DataEntrySet] = {
    ...
    val datasets: List[DataEntrySet] = dataFrames.map { ndf =>
      ...
      val dataEntries = df.schema.fields map { f =>
        ...
        val normalizedColName = DataFrameUtils.sanitizedName(f.name)
        val convertedDF = convertToNumberDataFrame(flattenDF.select(normalizedColName))
        //Remove all null and nan values and count how many were removed.
        //also remove "NaN" strings
        val filteredFlattenedDF = convertedDF.na.drop().filter(!convertedDF(normalizedColName).isNaN)

        val (featLens, counts, missingCount) =
          if (isNestedArrayType(f)) {
            val sequenceLengthDF  = df.select(size(df(f.name)))
            val valueLengthDF  :DataFrame   = featureDF.select(size(featureDF(normalizedColName)))
        ...
        DataEntry(featureName=normalizedColName,
          `type` = protoTypeName,
          values = convertedDF,
          counts = counts,
