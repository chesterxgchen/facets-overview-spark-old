...
object DataFrameUtils {
  ...
  def flattenDataFrame(df: DataFrame, recursive: Boolean = true): DataFrame = {
    var flattenedDf = df.select(flattenSchemaNames(df.schema)
                        .map(col => col.alias(sanitizedColName(col))): _*)
  ...
  private def sanitizedColName(col: Column) : String = {
    sanitizedName(col.toString())
  }

  private[spark] def sanitizedName(colName: String) :String = {
    colName.replace(".", "_").replace("-", "_").replace("__", "_").replaceFirst("^_", "")
  }
}
